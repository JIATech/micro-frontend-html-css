...
$(document).ready(function () {
    ...
    $("html,body").on(
        ...
        function (event) {
            var $currentSection = $(".slide.selected .content"),
                scrollsize = Math.ceil(
                    Math.abs(event.deltaY) * event.deltaFactor
                ),
            ...
            if (
                ...
                if (window.allowSlide && scrollsize) {
                    if (scrollDirection == "down") {
                        window.collectScrolls =
                            window.collectScrolls + scrollsize;
                    } else {
                        window.collectScrolls =
                            window.collectScrolls - scrollsize;
                    }
                ...
                if (halfOnScreen && allowToSelect) {
                    ...
                    if (
                        ...
                        if (window.isAnimated == "auto") {
                            //clearTimeout(window.clearElementAnimation);
                            window.clearElementAnimation = setTimeout(
                                function () {
                                    $element
                                        .find("[class*='ae-']")
                                        .addClass("done");
        ...
        $(".panel .compact").each(function (index, element) {
            ...
            $(window).on("load resize ready", function () {
                var documentWidth = $(document).width(),
                    panelsPadding =
                        parseInt(
                            $(panel).css("padding-left").replace("px", "")
                        ) +
                        parseInt(
                            $(panel).css("padding-right").replace("px", "")
                        );

                if (
                    (window.isMobile || $(document).width() < 767) &&
                    forceMobileView
                ) {
                    ...
                    if (
                        totalWidth + Math.round(panelsPadding) >
                            documentWidth + 2 ||
                        ((window.isMobile || documentWidth < 767) &&
                            forceMobileView)
                    ) {
                        $(desktop).addClass("hidden");
                        $(compact).removeClass("hidden");
    ...
    if ($(navigation).length > 0) {
        ...
        $(".navigation li").on("click touchend", function () {
            var thisIndes = $(this).index(),
                realIndex = $(
                    ".slide:not(.exclude):eq(" + thisIndes + ")"
                ).index(".slide");
            ...
            if ($(navigation).length > 0) {
                $(navigation).each(function (index, element) {
                    ...
                    var $selectedSlide = $(".slide.selected"),
                        parentSlide = parseInt(
                            $selectedSlide.data("parent-slide-id")
                        ),
                        selectedIndex = $selectedSlide.index(
                            ".slide:not(.exclude)"
                        );
    ...
    function hidePopup(popupID) {
        ...
        if ($.isArray(window.popupShown)) {
            var popupToHide = popupID
                    ? popupID
                    : window.popupShown.slice(-1)[0],
                $element = $(
                    '.popup.visible[data-popup-id="' + popupToHide + '"]'
                ),
                iframe = $element.find("iframe"),
            ...
            if ($el.hasClass("clickable") || $el.hasClass("autoplay")) {
                $el.on("click next", function (event) {
                    var $el = $(this),
                        ...
                        sliderID = $el.data("slider-id"),
                        $controller = $(
                            '.controller[data-slider-id="' + sliderID + '"]'
                        ),
    ...
    $("#contact-form, [data-ajax-form]").each(function (index, element) {
        $(element).ajaxForm(function () {
            var $ajaxForm = $(element),
                ...
                ajaxFormButtonIsInput = $ajaxFormButton.is("input")
                    ? true
                    : false,
    ...
    $(".soundTrigger").click(function () {
        var musicID = $(this).data("sound-id"),
            ...
            action = $audio.data("sound-action")
                ...
                : "toggle",
            fade =
                parseInt($audio.data("sound-fade")) >= 0 ||
                $audio.data("sound-fade")
